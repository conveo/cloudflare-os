...
const mcpPortalPaths = [
  "workers.mcpPortalGatekeeper.name", "mcpPortal.url", "mcpPortal.name",
];
...
export function validateConfig(config: DeploymentConfig): DeploymentConfig {
  ...
  const inactiveWorkers: Record<string, boolean> = {
  ...
  if (config.mcpPortal.enabled) {
    // The portal URL is every user's OAuth destination, so a typo sends the whole deployment's
    // sign-in flow at whatever host it names. Upstream hides an unusable URL rather than failing
    // loudly -- the connector simply stops appearing -- so reject it here instead.
    let portal: URL;
    try {
...
export function generateConfigs(config: DeploymentConfig, bases: BaseConfigs): GeneratedConfigs {
  ...
  router.services = [
    ...
    // Both MCP connectors serve HTTP: their OAuth callback lands on /gatekeeper/<name>/oauth,
    // which is the whole reason this deployment runs a router at all.
    ...(config.mcp.enabled
  ...
  workshop.services = [
    ...
    // The backend discovers Gatekeepers by scanning GATEKEEPER_* bindings, so binding a
  ...
  // Both connectors derive their own path prefix and OAuth redirect_uri from BASE_URL, so it
  // must equal the router path its binding name produces. publicOrigin() is what makes a
  // workers.dev deployment possible here at all -- the old script had to refuse one, because
  // the origin was unknowable before the first deploy.
  const mcpGatekeeper = config.mcp.enabled ? structuredClone(bases.mcpGatekeeper) : undefined;
  if (mcpGatekeeper) {
  ...
  if (mcpPortalGatekeeper) {
    ...
    mcpPortalGatekeeper.vars = {
      ...
      // Off deliberately: the portal fronts servers whose own destructiveHint/idempotentHint we
      // do not review, and trusting them would let any one of them self-declare its writes as
      // auto-approvable.
...
export function buildCommands(config: DeploymentConfig): BuildCommand[] {
  return [
    ...
    ...(config.mcp.enabled
      ? [{ args: submoduleBuild("@gadgets/mcp-gatekeeper", "build:configurator") }] : []),
    ...(config.mcpPortal.enabled
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (config.mcp.enabled) deployWorker(packageDirs.mcpGatekeeper, deployArgs);
    if (config.mcpPortal.enabled) deployWorker(packageDirs.mcpPortalGatekeeper, deployArgs);
