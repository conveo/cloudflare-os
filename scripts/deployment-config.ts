...
export interface DeploymentConfig {
  ...
  workers: {
    ...
    /** Only required when `mcpPortal.enabled`. */
  ...
  /**
   * Upstream's generic MCP connector: a user pastes any MCP endpoint and its tools become typed
   * capabilities. At Conveo that endpoint is agentgateway, so a call runs as the person making
   * it. See docs/agentgateway.md.
   */
  mcp: { enabled: boolean };
  /**
   * Upstream's MCP portal connector: one administrator-configured endpoint, so nobody pastes a
   * URL, and grants scope to a single server behind it.
   */
  mcpPortal: { enabled: boolean; url?: string; name?: string };
...
export interface GeneratedConfigs {
  ...
  /** Absent when `mcpPortal.enabled` is false. */
